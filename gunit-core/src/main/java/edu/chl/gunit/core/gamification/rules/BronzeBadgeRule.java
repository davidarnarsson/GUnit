 ...
 */
public class BronzeBadgeRule extends BadgeRule {

    @Inject
    public BronzeBadgeRule(UserService userService, UserBadgeService userBadgeService, BadgeService badgeService) {
    ...
    protected boolean shouldGetBadge(UserRecord user, GamificationContext ctx, RuleRecord self) {
        return user.getTotalwrittentests() + ctx.getStatistics().getNewTestCases().size() >= 10;
    }
