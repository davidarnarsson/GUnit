 ...
 */
public class SilverBadgeRule extends BadgeRule{

    @Inject
    public SilverBadgeRule(UserService userService, UserBadgeService userBadgeService, BadgeService badgeService) {
    ...
    protected boolean shouldGetBadge(UserRecord user, GamificationContext ctx, RuleRecord self) {
        return user.getTotalwrittentests() + ctx.getStatistics().getNewTestCases().size() >= 25;
    }
